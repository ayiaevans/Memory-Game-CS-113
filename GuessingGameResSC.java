...
class GuessingGameRes implements Game { //the "back-end" of the program 
   ...
   private int [][] board; 
   private boolean [][] faceup;//a seperate list that keeps the correct guesses 
   private int rows; 
   private int col; 
   int [] choice; 
   int [] cardvalue;
   ...
   public void setSizeChoice(int s){
      sizeChoice = s; }
   ...
   public int[][] cards(int [][]board){
      ...
      for(int r = 0; r<sizeChoice; r++){
         for(int c = 0; c<sizeChoice; c++){
            int num = cardvalue[i];
            num = num + 1;
            if(cardvalue[i]>sizeChoice*2-1){
               num = num - sizeChoice*2;}
            board[r][c]= num; 
            i++;}
            }
          return(board); 
  ...
  /* public int getSize(String size){
   ...
   for(int r = 0; r<sizeChoice; r++){
      for(int c = 0; c<sizeChoice; c++){
         if (faceup[r][c]==true){
            match++;
               if(match==sizeChoice*sizeChoice){
                  isWinner=true;}}
